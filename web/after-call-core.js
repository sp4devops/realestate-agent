...
function readPendingAfterCall(storage,nowMs=()=>Date.now()){
 const raw=storage.getItem(PENDING_KEY);if(!raw)return null;
 try{
  const payload=JSON.parse(raw);const launchedAt=Date.parse(payload?.launchedAt||'');const age=nowMs()-launchedAt;
  if(!Number.isFinite(launchedAt)||age<0||age>PENDING_MAX_AGE_MS){storage.removeItem(PENDING_KEY);return null;}
  return payload;
 }catch(_){storage.removeItem(PENDING_KEY);return null;}
}
...
root.PropertyAssistantAfterCall={PENDING_KEY,PENDING_MAX_AGE_MS,normalizePhone,phoneForWhatsApp,validateRecap,buildRecords,findPersonByPhone,resolveFollowUpPhone,transition,markPendingAfterCall,readPendingAfterCall,clearPendingAfterCall,createCommunicationService};
})(globalThis);
