(function () {
  ...
  function peopleOptions(){
    return window.__PA_REPOSITORY__.list('people').sort((a,b)=>a.name.localeCompare(b.name)).map(person=>({value:person.id,label:`${person.name} · ${person.primaryPhone || 'phone pending'}`}));
  }
  ...
  function suggestedPersonId(parsed){
    const normalize=window.PropertyAssistantPersistence.normalizePhone; const phone=normalize(parsed.person?.primaryPhone); const name=String(parsed.person?.name || '').trim().toLowerCase();
    ...
    const names=people.filter(person=>name&&person.name.trim().toLowerCase()===name); return names.length===1?names[0].id:'';
  }
  ...
  function reviewBody(parsed){
    ...
    if(parsed.kind==='property') return `${uncertain}<form data-testid="review-form">${field('Owner name','name',parsed.person?.name || '')}${field('Primary phone (optional)','primaryPhone',parsed.person?.primaryPhone || '')}${field('Property type','propertyType',parsed.property?.propertyType || '')}${sizeFields(parsed.property?.size)}${field('Locality','locality',parsed.property?.locality || '')}${field('Intent','intent',parsed.property?.intent || 'sale')}${field('Price / rate','price',money(parsed.property?.price),'number')}${selectField('Price basis','priceBasis',[{value:'total',label:'Total price'},{value:'per_month',label:'Per month'},{value:'per_acre',label:'Per acre'},{value:'per_cent',label:'Per cent'},{value:'per_sqft',label:'Per sqft'}],parsed.property?.priceBasis || 'total',false)}${field('Attributes (comma separated)','attributes',listValue(parsed.property?.attributes))}<div class="page-actions"><button class="button primary" type="submit" data-testid="save-capture">Save property</button><button class="button" type="button" data-route="type">Edit note</button></div></form>`;
    if(parsed.kind==='requirement') return `${uncertain}<form data-testid="review-form">${field('Name','name',parsed.person?.name || '')}${field('Primary phone (optional)','primaryPhone',parsed.person?.primaryPhone || '')}${field('Property type','propertyType',parsed.requirement?.propertyType || '')}${sizeFields(parsed.requirement?.size,true)}${field('Locality','locality',parsed.requirement?.locations?.[0] || '')}${field('Intent','intent',parsed.requirement?.intent || 'buy')}<div class="field-row">${field('Budget from','budgetMin',money(parsed.requirement?.budgetMin),'number')}${field('Budget up to','budgetMax',money(parsed.requirement?.budgetMax),'number')}</div>${field('Move / decision timing','timing',parsed.requirement?.timing || '')}${field('Preferences (comma separated)','preferences',listValue(parsed.requirement?.preferences))}<div class="page-actions"><button class="button primary" type="submit" data-testid="save-capture">Save requirement</button><button class="button" type="button" data-route="type">Edit note</button></div></form>`;
    ...
    if(parsed.kind==='followup') return `${uncertain}<form data-testid="review-form">${selectField('Existing person (optional)','targetPerson',peopleOptions(),suggestedPersonId(parsed))}${field('Name','name',parsed.person?.name || '')}${field('Primary phone (optional)','primaryPhone',parsed.person?.primaryPhone || '')}${field('Follow-up title','title',parsed.followUp?.title || 'Follow up')}${field('Due at','dueAt',dueLocal(parsed.followUp?.dueText),'datetime-local')}<div class="page-actions"><button class="button primary" type="submit" data-testid="save-capture">Save follow-up</button><button class="button" type="button" data-route="type">Edit note</button></div></form>`;
    return `${uncertain}<form data-testid="review-form">${selectField('Person','targetPerson',peopleOptions(),suggestedPersonId(parsed))}${field('Name','name',parsed.person?.name || '')}${field('Primary phone (optional)','primaryPhone',parsed.person?.primaryPhone || '')}${selectField('Related property (optional)','targetProperty',propertyOptions(),suggestedPropertyId(parsed))}${textArea('What happened','summary',parsed.interaction?.summary || parsed.source)}${field('Preferences learned (comma separated)','learnedPreferences',listValue(parsed.interaction?.learnedPreferences))}<div class="page-actions"><button class="button primary" type="submit" data-testid="save-capture">Remember update</button><button class="button" type="button" data-route="type">Edit note</button></div></form>`;
  }
  ...
  function findPerson(tx,{phone='',name=''}){
    ...
    const candidates=people.filter(person=>normalizeName(person.name)===normalizeName(name) && (!target || !normalize(person.primaryPhone)));
  ...
  function finish(){ sessionStorage.removeItem(DRAFT_KEY); }
  ...
  function saveRequirement(form,parsed){
    ...
    const result=repo.transact(tx=>{
      ...
      const person=upsertPerson(tx,{name:String(form.get('name')||'').trim(),phone,role});
  ...
  function saveProperty(form,parsed){
    ...
    const result=repo.transact(tx=>{
      ...
      const owner=ownerName?upsertPerson(tx,{name:ownerName,phone:ownerPhone,role:'owner'}):null;
